...
to setup
  ...
  set show-water? true
  ask patches [
    set elevation 20 + 4 * abs pycor + random (pxcor + min-pxcor + 5)
    set drain? false
  ]
  repeat terrain-smoothness [ diffuse elevation 0.5 ]
 
  ;; make the drain at middle left
  ask patches with [pxcor = min-pxcor  and abs pycor < .05 * max-pycor ]
    [ set drain? true
      set elevation -1 ]
...
to flow  ;; patch procedure
  ...
  if (elevation < 0) [ set drain? true set drains (patch-set drains self) ]
end

to export-terrain
  file-open "terrain.csv"
  ask patches [ file-print (word "[" pxcor " " pycor " " elevation "]") ]
  file-close-all
end
