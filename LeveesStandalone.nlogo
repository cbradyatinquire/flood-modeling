...
to export-current-water-levels
  file-open "water-levels.csv"
  ask patches [
   file-print (word "[" pxcor " " pycor " " elevation " " water " " am-edge? "]")
  ]
  file-close-all
end
...
to import-water-levels
  ...
  if (f != false) [
    ...
    while [not file-at-end? ] [
     ...
     let px item 0 line
     let py item 1 line
     let pwater item 3 line
     let pam-edge item 4 line 
     ask patch px py [ set water pwater set am-edge? pam-edge ]
    ] 
    file-close-all
  ]
end
